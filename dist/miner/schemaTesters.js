 ...
 */
function regionFullyInsideRows(region, rows, windowHeight, width) {
 ...
 */
function getRegionsIntersectingRows(state, rows, width) {
 ...
 */
function getCandidatesInRegionAndRows(region, rows, state, width) {
    const rowSet = new Set(rows);
    const candidates = [];
    for (const cellId of region.cells) {
        const row = Math.floor(cellId / width);
 ...
 */
export function testA1Preconditions(state, window) {
    if (window.height < 2) {
        return { holds: false };
    }
 ...
 */
export function testA2Preconditions(state, window) {
    if (window.width < 2) {
        return { holds: false };
    }
    if (state.regions.length < 2) {
 ...
 */
function countValidBlocks(state, window) {
    let count = 0;
    ...
    for (let r = 0; r < height - 1; r++) {
        for (let c = 0; c < width - 1; c++) {
            ...
            const hasCandidate = state.cellStates[topLeft] === CellState.Unknown ||
 ...
 */
export function testC2Preconditions(state, window) {
 ...
 */
export function testC1Preconditions(state, window) {
    if (window.width < 2 || window.height < 2) {
        return { holds: false };
    }
    const validBlocks = countValidBlocks(state, window);
 ...
 */
export function testE1Preconditions(state, window) {
    // Check rows
 ...
 */
export function testD1Preconditions(state, window) {
 ...
 */
export function testSchemaPreconditions(familyId, state, window) {
    switch (familyId) {
...
//# sourceMappingURL=schemaTesters.js.map