...
function isValidCompletion(state: BoardState): boolean {
  ...
  for (const row of state.rows) {
    const starCount = row.cells.filter((cellId: number) => state.cellStates[cellId] === CellState.Star).length;
  ...
  for (const col of state.cols) {
    const starCount = col.cells.filter((cellId: number) => state.cellStates[cellId] === CellState.Star).length;
  ...
  for (const region of state.regions) {
    const starCount = region.cells.filter((cellId: number) => state.cellStates[cellId] === CellState.Star).length;
...
function printBoard(state: BoardState) {
  const width = state.windowWidth || state.size;
  const height = state.windowHeight || state.size;
...
async function test4x4Completion() {
  ...
  for (const row of solution.rows) {
    const count = row.cells.filter((c: number) => solution.cellStates[c] === CellState.Star).length;
    console.log(`  Row ${row.rowIndex}: ${count}/${row.starsRequired} ${count === row.starsRequired ? '✓' : '✗'}`);
  }
  // Check cols
  for (const col of solution.cols) {
    const count = col.cells.filter((c: number) => solution.cellStates[c] === CellState.Star).length;
    console.log(`  Col ${col.colIndex}: ${count}/${col.starsRequired} ${count === col.starsRequired ? '✓' : '✗'}`);
  }
  // Check regions
  for (const region of solution.regions) {
    const count = region.cells.filter((c: number) => solution.cellStates[c] === CellState.Star).length;
