...
function checkConstraints(state: any, width: number, height: number) {
  ...
  for (const row of testState.rows) {
    const starCount = row.cells.filter((c: number) => testState.cellStates[c] === CellState.Star).length;
  ...
  for (const col of testState.cols) {
    const starCount = col.cells.filter((c: number) => testState.cellStates[c] === CellState.Star).length;
  ...
  for (const region of testState.regions) {
    const starCount = region.cells.filter((c: number) => testState.cellStates[c] === CellState.Star).length;
