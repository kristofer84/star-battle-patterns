 ...
 */
export async function testSimpleE1Pattern(): Promise<boolean> {
  ...
  const row0Candidates = testState.rows[0].cells.filter(
    c => testState.cellStates[c] === CellState.Unknown
  );
  ...
  const row0Forced = testState.rows[0].cells.filter(
    c => analysis.cellResults.get(c) === 'alwaysStar'
  );
